    ...
    try:
        with KuzuMemory(db_path=db_path) as memory:
            ...
            auggie = AuggieIntegration(
                kuzu_memory=memory, config={"max_context_memories": 8, "enable_learning": True}
            )

            print("✅ Initialized KuzuMemory with Auggie integration")
            ...
            for scenario in learning_scenarios:
                ...
                print(
                    f"   - Memories extracted: {len(learning_result.get('extracted_memories', []))}"
                )
            ...
            print(
                f"  - Average quality: {stats['response_learner'].get('average_quality_score', 0):.2f}"
            )
            ...
            sorted_rules = sorted(
                rule_stats.items(), key=lambda x: x[1]["execution_count"], reverse=True
            )

            for rule_id, rule_info in sorted_rules[:3]:
                print(
                    f"  - {rule_info['name']}: {rule_info['execution_count']} executions, "
            ...
            print("   ✅ Intelligent prompt enhancement based on user context")
            print("   ✅ Automatic learning from AI responses and user feedback")
            print("   ✅ Dynamic rule creation and application")
            print("   ✅ Comprehensive monitoring and statistics")
            print("   ✅ Rule persistence and sharing")
