...
class TestSummaryGenerator:
    ...
    def parse_benchmark_results(self, benchmark_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "parse_error", "error": str(e), "benchmarks": []}

    def aggregate_results(
        self,
        unit_results: list[Path],
        integration_results: Path | None,
        e2e_results: Path | None,
...
<body>
    ...
    def generate_markdown_report(self, output_file: Path):
        ...
        for suite_name, suite_data in self.summary_data["suites"].items():
            if suite_data.get("status") == "completed":
                passed = (
                    suite_data.get("tests", 0)
                    - suite_data.get("failures", 0)
        ...
        perf_data = self.summary_data.get("performance", {})
        if perf_data.get("status") == "completed" and perf_data.get("benchmarks"):
            markdown_lines.extend(["## 📊 Performance Benchmarks", ""])
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("test-summary.html"), help="Output HTML file"
    )
