...
class PerformanceChecker:
    ...
    def _check_benchmark_regression(
        self, benchmark_name: str, stats: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _report_results(self, regressions: list[dict], improvements: list[dict]):
        ...
        if regressions:
            ...
            for regression in regressions:
                ...
                print(
                    f"    Mean: {regression['current_mean']:.2f}ms (baseline: {regression['baseline_mean']:.2f}ms)"
                )
                if regression.get("mean_regression"):
                    print(
                        f"    ⚠️  Mean time regression: {regression['current_mean'] / regression['baseline_mean']:.2f}x"
                    )

                print(
                    f"    P95:  {regression['current_p95']:.2f}ms (baseline: {regression['baseline_p95']:.2f}ms)"
                )
                if regression.get("p95_regression"):
                    print(
                        f"    ⚠️  P95 time regression: {regression['current_p95'] / regression['baseline_p95']:.2f}x"
                    )
                print()
        ...
        if improvements:
            ...
            for improvement in improvements:
                ...
                print(
                    f"    Mean: {improvement['current_mean']:.2f}ms (baseline: {improvement['baseline_mean']:.2f}ms)"
                )
                print(
                    f"    🚀 Improvement: {improvement['baseline_mean'] / improvement['current_mean']:.2f}x faster"
                )
                print()
...
def main():
    ...
    parser.add_argument(
        "--update-baseline", action="store_true", help="Update baseline with improvements"
    )
