        ...
        try:
            with KuzuMemory(db_path=db_path) as memory:
                ...
                for query in test_queries:
                    ...
                    print(f"  🔍 Query: {query}")
                    print(f"     Memories: {len(context.memories)}, Confidence: {context.confidence:.2f}")
                ...
                avg_recall_time = sum(r["recall_time_ms"] for r in recall_results) / len(recall_results)
                ...
                print(f"  Target: <10ms average, <20ms max")
                ...
                print(f"  Average confidence: {sum(r['confidence'] for r in recall_results) / len(recall_results):.2f}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            with KuzuMemory(db_path=db_path) as memory:
                ...
                print(f"✅ Auggie integration initialized")
