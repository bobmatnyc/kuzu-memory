    ...
    def test_memory_content_integrity(
        ...
        with KuzuMemory(db_path=temp_db_path, config=integrity_config) as memory:
            ...
            for memory_id in stored_memory_ids:
                ...
                hashlib.sha256(retrieved_memory.content.encode("utf-8")).hexdigest()
                original_content = next(
                    (
                        data["content"]
                        for data in reference_dataset
                        if data["content"] in retrieved_memory.content
                        or retrieved_memory.content in data["content"]
                    ),
                    None,
                )
            ...
            recall_queries = [
                (
                    "sarah-chen",
                    "DataTech Solutions",
                ),  # Matches "works at DataTech Solutions"
                (
                    "sarah-chen",
                    "Python TensorFlow",
                ),  # Matches "prefers Python...TensorFlow"
                (
                    "sarah-chen",
                    "MySQL PostgreSQL",
                ),  # Matches "migrate from MySQL to PostgreSQL"
                (
                    "sarah-chen",
            ...
            for user_id, query in recall_queries:
                ...
                assert (
                    len(context.memories) > 0
                ), f"Failed to recall memories for query: {query}"

                # Verify content in enhanced prompt
                assert len(context.enhanced_prompt) > len(context.original_prompt)

    def test_memory_type_classification_integrity(
        self, temp_db_path, integrity_config, reference_dataset
    ):
    ...
    def test_recall_consistency_integrity(
        ...
        with KuzuMemory(db_path=temp_db_path, config=integrity_config) as memory:
            ...
            for user_id, query in test_queries:
                ...
                consistent = all(
                    result["memory_ids"] == first_result["memory_ids"]
                    and abs(result["confidence"] - first_result["confidence"]) < 0.01
            ...
            print(
                f"  Consistent queries: {consistent_queries}/{total_queries} ({consistency_rate:.1%})"
            )
        ...
        with KuzuMemory(db_path=temp_db_path, config=integrity_config) as memory2:
            ...
            for _content, data in stored_data.items():
                ...
                for memory_id in data["memory_ids"]:
                    ...
                    if (
                        not retrieved_memory.content
                        or len(retrieved_memory.content.strip()) == 0
                    ):
                        persistence_integrity = False
        ...
        def worker(worker_id):
            try:
                with KuzuMemory(
                    ...
                    context = memory.attach_memories(
                        "What do I work on?", user_id=user_id, max_memories=5
                    )
