...
@pytest.mark.skipif(os.getenv("CI"), reason="Stress test - skip in CI")
        ...
        with KuzuMemory(db_path=temp_db_path, config=stress_config) as memory:
            ...
            for content in base_content:
                memory.generate_memories(
                    content=content, user_id=user_id, source="stress_test_data"
                )
            ...
            for worker_id in range(num_workers):
                ...
                worker_queries = [
                    (i, random.choice(test_queries)) for i in range(queries_per_worker)
                ]

                thread = threading.Thread(
                    target=read_worker, args=(worker_id, worker_queries, results_queue)
                )
    ...
    def test_mixed_concurrent_operations(self, temp_db_path, stress_config):
        ...
        with KuzuMemory(db_path=temp_db_path, config=stress_config) as memory:
            ...
            for worker_id in range(read_workers):
                thread = threading.Thread(
                    target=read_worker, args=(worker_id, operations_per_worker)
                )
                threads.append(thread)
                thread.start()
            ...
            for worker_id in range(write_workers):
                thread = threading.Thread(
                    target=write_worker, args=(worker_id, operations_per_worker)
                )
                threads.append(thread)
                thread.start()
        ...
        with KuzuMemory(db_path=temp_db_path, config=stress_config) as memory:
            ...
            if performance_results:
                ...
                recall_times_by_count = [
                    performance_results[count]["avg_recall_time"] for count in sorted_counts
                ]
                ...
                if len(recall_times_by_count) >= 2:
                    ...
                    if first_recall > 1.0:  # At least 1ms for meaningful comparison
                        ...
                        assert max_recall < 1000.0, f"Max recall time {max_recall:.2f}ms too high"
                        print("Memory Pressure Analysis:")
