...
class TestAutoRepairEndToEnd:
    ...
    def test_auto_repair_multiple_projects(self, backup_claude_config, tmp_path):
        ...
        assert after_config["mcpServers"]["kuzu-memory"]["args"] == ["mcp"]
        assert (
            after_config["projects"]["/Users/test/project1"]["mcpServers"]["kuzu-memory"]["args"]
            == ["mcp"]
        )
        assert (
            after_config["projects"]["/Users/test/project2"]["mcpServers"]["kuzu-memory"]["args"]
            == ["mcp"]
        )
    ...
    def test_auto_repair_preserves_other_servers(self, backup_claude_config):
        ...
        assert after_config["mcpServers"]["another-mcp-server"]["args"] == ["mcp", "serve"]
    ...
    def test_auto_repair_with_extra_args(self, backup_claude_config, tmp_path):
        ...
        config = {
            "mcpServers": {
                "kuzu-memory": {
                    "command": "kuzu-memory",
                    "args": ["mcp", "serve", "--debug"],  # Extra arg should be preserved
                }
            }
        }
