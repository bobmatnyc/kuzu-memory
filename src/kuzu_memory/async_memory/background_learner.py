...
from .queue_manager import MemoryQueueManager, MemoryTask, TaskType, TaskStatus, get_queue_manager
...
class BackgroundLearner:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        logger.info("Initialized BackgroundLearner")
    ...
    def learn_async(
        ...
        if success:
            logger.debug(f"Submitted learning task {task.task_id}")
            return task.task_id
        else:
    ...
    def _process_learn_task(self, task: MemoryTask) -> Dict[str, Any]:
        ...
        try:
            ...
            with KuzuMemory(db_path=self.db_path) as memory:
                ...
                logger.debug(f"Learned {len(memory_ids)} memories from task {task.task_id}")
...
def get_background_learner(db_path: Optional[Path] = None) -> BackgroundLearner:
