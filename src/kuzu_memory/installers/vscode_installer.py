"""
VS Code MCP installer for KuzuMemory.
...
class VSCodeInstaller(BaseInstaller):
    """
    Installer for VS Code MCP integration.

    Creates .vscode/mcp.json configuration file with KuzuMemory MCP server.
    Preserves existing MCP servers in the configuration.

    Note: VS Code may use "servers" key instead of "mcpServers" in some versions.
    This installer supports both formats.
    """

    @property
    def ai_system_name(self) -> str:
        """Name of the AI system this installer supports."""
        return "VS Code (Claude Extension)"
    ...
    def _normalize_config_format(self, config: dict) -> dict:
        """
        Normalize VS Code MCP configuration format.

        VS Code may use either "mcpServers" or "servers" key.
        ...
        Returns:
            Normalized configuration with "mcpServers" key
        """
        if "servers" in config and "mcpServers" not in config:
            # Convert "servers" to "mcpServers"
            config = config.copy()
            config["mcpServers"] = config.pop("servers")
            self.warnings.append("Converted VS Code 'servers' format to 'mcpServers'")
    ...
    def install(
        ...
        try:
            ...
            if existing_config:
                existing_config = self._normalize_config_format(existing_config)
