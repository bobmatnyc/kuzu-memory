...
class ClaudeHooksInstaller(BaseInstaller):
    ...
    def _create_claude_code_config(self) -> dict[str, Any]:
        ...
        - Stop: Learns from conversations asynchronously

        Hook variables available:
        - {{prompt}}: User's prompt text (UserPromptSubmit event)
        - {{response}}: Claude's response text (Stop event)

        Also configures the kuzu-memory MCP server for tool access.
        ...
        config = {
            "_comment": f"Generated by KuzuMemory v{pkg_version} with correct Claude Code hook events (UserPromptSubmit, PostToolUse)",
            "hooks": {
                "UserPromptSubmit": [
                    {
                ...
                "PostToolUse": [
                    {
                        ...
                        "command": f'{kuzu_cmd} memory learn "{{{{content}}}}" --quiet',
                        "enabled": True,
                    }
                ],
            },
...
"""
    ...
    def install(
        ...
        try:
            ...
            self._validate_hook_events(existing_config)
