...
class DeduplicationEngine:
    ...
    def find_duplicates(
        ...
        for memory in memories_to_check:
            ...
            if (len(new_content_clean) < self.min_length_for_similarity or 
                len(memory.content) < self.min_length_for_similarity):
                continue
            
            # Layer 2: Normalized text comparison
            memory_normalized = self._normalize_text(memory.content)
            normalized_similarity = self._calculate_text_similarity(new_normalized, memory_normalized)
