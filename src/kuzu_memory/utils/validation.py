...
def validate_database_path(db_path: Union[str, Path]) -> Path:
    """
    ...
    if len(str(abs_path)) > MAX_PATH_LENGTH:
        raise ValidationError("db_path", str(abs_path), f"exceeds maximum length of {MAX_PATH_LENGTH}")
    
    # Check if parent directory is writable (if it exists)
    parent_dir = abs_path.parent
    if parent_dir.exists() and not parent_dir.is_dir():
        raise ValidationError("db_path", str(parent_dir), "parent path exists but is not a directory")
...
def validate_config_dict(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "storage" in config:
        ...
        if not isinstance(storage_config, dict):
            raise ValidationError("config.storage", str(type(storage_config)), "must be a dictionary")
        
        if "max_size_mb" in storage_config:
            max_size = storage_config["max_size_mb"]
            if not isinstance(max_size, (int, float)) or max_size <= 0:
                raise ValidationError("config.storage.max_size_mb", str(max_size), "must be a positive number")
    ...
    if "recall" in config:
        ...
        if "max_memories" in recall_config:
            ...
            if not isinstance(max_memories, int) or max_memories <= 0:
                raise ValidationError("config.recall.max_memories", str(max_memories), "must be a positive integer")
