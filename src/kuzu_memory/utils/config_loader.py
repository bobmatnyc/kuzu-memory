...
class ConfigLoader:
    ...
    def _auto_discover_config(self) -> Optional[Dict[str, Any]]:
    ...
    def _load_from_environment(self) -> Optional[Dict[str, Any]]:
        ...
        env_mappings = {
            ...
            f"{self.env_prefix}DB_AUTO_COMPACT": ("storage.auto_compact", self._parse_bool),
            f"{self.env_prefix}DB_CONNECTION_POOL_SIZE": ("storage.connection_pool_size", int),
            ...
            f"{self.env_prefix}RECALL_DEFAULT_STRATEGY": ("recall.default_strategy", str),
            f"{self.env_prefix}RECALL_ENABLE_CACHING": ("recall.enable_caching", self._parse_bool),
            f"{self.env_prefix}RECALL_CACHE_SIZE": ("recall.cache_size", int),
            f"{self.env_prefix}RECALL_CACHE_TTL_SECONDS": ("recall.cache_ttl_seconds", int),
            f"{self.env_prefix}EXTRACTION_MIN_LENGTH": ("extraction.min_memory_length", int),
            f"{self.env_prefix}EXTRACTION_MAX_LENGTH": ("extraction.max_memory_length", int),
            f"{self.env_prefix}EXTRACTION_ENABLE_ENTITIES": ("extraction.enable_entity_extraction", self._parse_bool),
            f"{self.env_prefix}EXTRACTION_COMPILE_PATTERNS": ("extraction.enable_pattern_compilation", self._parse_bool),
            f"{self.env_prefix}PERFORMANCE_MAX_RECALL_TIME_MS": ("performance.max_recall_time_ms", float),
            f"{self.env_prefix}PERFORMANCE_MAX_GENERATION_TIME_MS": ("performance.max_generation_time_ms", float),
            f"{self.env_prefix}PERFORMANCE_ENABLE_MONITORING": ("performance.enable_performance_monitoring", self._parse_bool),
            f"{self.env_prefix}PERFORMANCE_LOG_SLOW_OPS": ("performance.log_slow_operations", self._parse_bool),
            f"{self.env_prefix}RETENTION_ENABLE_AUTO_CLEANUP": ("retention.enable_auto_cleanup", self._parse_bool),
            f"{self.env_prefix}RETENTION_CLEANUP_INTERVAL_HOURS": ("retention.cleanup_interval_hours", int),
            f"{self.env_prefix}RETENTION_MAX_TOTAL_MEMORIES": ("retention.max_total_memories", int),
        }
...
def load_config_from_file(config_path: Union[str, Path]) -> KuzuMemoryConfig:
    """
