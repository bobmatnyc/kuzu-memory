...
from .strategies import KeywordRecallStrategy, EntityRecallStrategy, TemporalRecallStrategy
...
class RecallCoordinator:
            ...
            if strategy_name in self.strategies:
                try:
                    strategy_memories = self.strategies[strategy_name].recall(
                        prompt, max_memories * 2,  # Get more to allow for ranking
    ...
    def _single_strategy_recall(
        ...
        if strategy_name not in self.strategies:
            raise RecallError(prompt, f"Unknown strategy: {strategy_name}")
    ...
    def _update_coordinator_stats(self, strategy_used: str, recall_time_ms: float) -> None:
        ...
        total_time = (self._coordinator_stats['avg_recall_time_ms'] * 
                     (self._coordinator_stats['total_recalls'] - 1) + recall_time_ms)
        self._coordinator_stats['avg_recall_time_ms'] = total_time / self._coordinator_stats['total_recalls']
