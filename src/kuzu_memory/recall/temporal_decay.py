...
class TemporalDecayEngine:
    ...
    def calculate_temporal_score(self, memory: Memory, current_time: Optional[datetime] = None) -> float:
        """
        ...
        if memory.created_at is None:
            logger.warning(f"Memory {memory.id} has None created_at, using current time")
        ...
        if (self.decay_config['enable_adaptive_decay'] and 
        ...
        # Apply bounds
        min_score = params.get('minimum_score', self.decay_config['minimum_decay_score'])
        max_score = params.get('maximum_score', self.decay_config['maximum_decay_score'])
    ...
    def get_decay_explanation(self, memory: Memory, current_time: Optional[datetime] = None) -> Dict[str, Any]:
        """
        ...
        recent_boost_applied = (
            self.decay_config['enable_adaptive_decay'] and 
            age_hours < self.decay_config['boost_recent_threshold_hours']
        )
        ...
        return {
            ...
            'decay_function': params.get('decay_function', DecayFunction.EXPONENTIAL).value,
    ...
    def get_effective_weight(self, memory: Memory, current_time: Optional[datetime] = None) -> float:
        """
