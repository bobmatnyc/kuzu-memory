...
from ..core.models import MemoryType, ExtractedMemory
...
class PatternExtractor:
    ...
    def _define_patterns(self) -> None:
        ...
        self.REMEMBER_PATTERNS = [
            ...
            (r"[Ff]or (?:future )?reference[,:]?\s*(.*?)(?:\.|$|!|\?)", 0.90, "for_reference"),
        ...
        self.IDENTITY_PATTERNS = [
            ...
            (r"I (?:work at|work for|am at|am with) ([A-Z][a-zA-Z0-9\s&.,'-]+?)(?:\.|$|,|\s+as)", 0.95, "work_at"),
            (r"I am (?:a|an) ([a-zA-Z\s]+?)(?:\.|$|,|\s+at)", 0.90, "i_am_role"),
            (r"I'?m (?:a|an) ([a-zA-Z\s]+?)(?:\.|$|,|\s+at)", 0.90, "im_role"),
            (r"I (?:live in|am from|am based in) ([A-Z][a-zA-Z\s,]+?)(?:\.|$)", 0.90, "location"),
            (r"[Mm]y (?:role|position|job|title) is ([a-zA-Z\s]+?)(?:\.|$|,)", 0.90, "my_role"),
        ...
        self.DECISION_PATTERNS = [
            (r"[Ww]e (?:decided|agreed|chose) (?:to |on |that )?(.*?)(?:\.|$|,)", 0.95, "we_decided"),
            (r"[Ll]et's (?:go with|use|choose|implement) (.*?)(?:\.|$|,)", 0.90, "lets_use"),
            (r"[Ww]e'?(?:ll|re going to) (?:use|go with|implement|choose) (.*?)(?:\.|$|,)", 0.90, "well_use"),
            (r"[Tt]he (?:decision|choice) (?:is|was) (?:to )?(.*?)(?:\.|$|,)", 0.90, "decision_is"),
            (r"[Ww]e should (?:use|go with|implement|choose) (.*?)(?:\.|$|,)", 0.85, "we_should"),
            (r"[Ii]t'?s (?:been )?decided (?:that )?(.*?)(?:\.|$|,)", 0.90, "its_decided"),
            (r"[Aa]fter discussion[,]? (?:we )?(.*?)(?:\.|$)", 0.85, "after_discussion"),
        ]
        ...
        self.STATUS_PATTERNS = [
            (r"[Cc]urrently (?:working on |doing |implementing )(.*?)(?:\.|$|,)", 0.80, "currently"),
            (r"[Rr]ight now (?:I'?m |we'?re )?(.*?)(?:\.|$|,)", 0.75, "right_now"),
            (r"[Aa]t the moment (.*?)(?:\.|$|,)", 0.75, "at_moment"),
            (r"[Tt]oday (?:I |we )?(.*?)(?:\.|$|,)", 0.70, "today"),
            (r"[Tt]his (?:week|month) (?:I'?m |we'?re )?(.*?)(?:\.|$|,)", 0.70, "this_period"),
    ...
    def extract_memories(self, text: str) -> List[ExtractedMemory]:
        ...
        try:
            ...
            for pattern_group, memory_type in patterns_to_use:
                group_matches = self._extract_from_pattern_group(clean_text, pattern_group, memory_type)
    ...
    def _get_runtime_patterns(self) -> List[Tuple[List[Tuple], MemoryType]]:
    ...
    def _is_common_phrase(self, content: str) -> bool:
        ...
        common_phrases = {
            'thank you', 'thanks', 'please', 'yes', 'no', 'ok', 'okay',
            'sure', 'alright', 'got it', 'i see', 'i understand',
            'hello', 'hi', 'bye', 'goodbye', 'see you', 'talk soon'
        }
