...
class RelationshipDetector:
    ...
    def _define_relationship_patterns(self) -> None:
        ...
        self.MEMORY_RELATIONSHIP_PATTERNS = [
            ...
            (r'(?:actually|no|wait|correction)[,]?\s*(.*?)(?:\.|$)', 'contradicts', 0.95),
            (r'(?:that\'s wrong|incorrect|not right)[,]?\s*(.*?)(?:\.|$)', 'contradicts', 0.90),
            ...
            # Reference patterns
            (r'(?:as mentioned|as I said|like I told you)[,]?\s*(.*?)(?:\.|$)', 'references', 0.80),
            (r'(?:similar to|like|just like)[,]?\s*(.*?)(?:\.|$)', 'similar_to', 0.75),
            
            # Elaboration patterns
            (r'(?:also|additionally|furthermore|moreover)[,]?\s*(.*?)(?:\.|$)', 'elaborates', 0.70),
        ...
        self.ENTITY_RELATIONSHIP_PATTERNS = [
            ...
            (r'(\w+)\s+(?:written in|implemented in|coded in)\s+(\w+)', 'implemented_in', 0.90),
    ...
    def get_relationship_statistics(self) -> Dict[str, any]:
        ...
        return {
            'total_patterns': len(self.compiled_memory_patterns) + len(self.compiled_entity_patterns),
