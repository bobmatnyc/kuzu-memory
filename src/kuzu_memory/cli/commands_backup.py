    ...
    try:
        ...
        if RICH_AVAILABLE:
            if db_path.exists():
                if not Confirm.ask(f"Database {db_path} exists. Continue with existing?"):
                    db_path = Path(Prompt.ask("Enter new database path", default="my_memories_new.db"))
        ...
        with KuzuMemory(db_path=db_path, config=config) as memory:
            ...
            for query in test_queries:
                ...
                if context.memories:
                    rich_print(f"     Top result: {context.memories[0].content[:60]}...", style="dim")
            ...
            if not skip_demo:
                ...
                try:
                    ...
                    rich_print(f"  🚀 Enhanced: {len(enhancement['enhanced_prompt'])} chars (was {len(test_prompt)})", style="green")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with KuzuMemory(db_path=db_path, config=config) as memory:
                ...
                for memory_text in demo_memories:
                    memory_ids = memory.generate_memories(memory_text, user_id="demo-user")
                ...
                try:
                    ...
                    rich_print(f"  🔍 Original: {test_prompt}", style="cyan")
                    rich_print(f"  🚀 Enhanced with personal context ({len(enhancement['enhanced_prompt'])} chars)", style="green")
                    rich_print(f"  📊 Added context about: FastAPI, PostgreSQL, pytest, Docker", style="dim")
...
def init(ctx, force, config_path):
    ...
    try:
        ...
        rich_print(f"🏗️  Initializing project memories in: {project_root.name}", style="bold blue")
        ...
        rich_print(f"🗄️  Initializing memory database...", style="blue")
...
def project(ctx, verbose):
    ...
    try:
                ...
                if verbose:
                    try:
                        ...
                        if 'database_stats' in stats.get('storage_stats', {}):
                            ...
                            rich_print(f"  • Memories: {db_stats.get('memory_count', 0)}", style="cyan")
                            rich_print(f"  • Entities: {db_stats.get('entity_count', 0)}", style="cyan")
                            rich_print(f"  • Sessions: {db_stats.get('session_count', 0)}", style="cyan")

                        perf_stats = stats.get('performance_stats', {})
                        rich_print(f"  • Recall calls: {perf_stats.get('attach_memories_calls', 0)}", style="cyan")
                        rich_print(f"  • Avg recall time: {perf_stats.get('avg_attach_time_ms', 0):.1f}ms", style="cyan")

                    except Exception as e:
                        rich_print(f"⚠️  Could not load memory statistics: {e}", style="yellow")
            else:
                rich_print("⚠️  Memory directory exists but database not initialized", style="yellow")
        ...
        if verbose:
            ...
            rich_print("  • Store project context: kuzu-memory remember 'project info'", style="cyan")
            rich_print("  • Find relevant info: kuzu-memory recall 'how does X work?'", style="cyan")
            rich_print("  • AI enhancement: kuzu-memory auggie enhance 'your prompt'", style="cyan")
...
@click.option('--format', 'output_format', default='context',
    ...
    try:
        ...
        with KuzuMemory(db_path=db_path) as memory:
            ...
            if output_format == 'json':
                ...
                if context.memories:
                    rich_print(f"🧠 Enhanced with {len(context.memories)} memories (confidence: {context.confidence:.2f})", style="green")
                    click.echo()
                    click.echo(context.enhanced_prompt)
                else:
...
@click.option('--sync', 'use_sync', is_flag=True, help='Use synchronous processing (blocking, for testing)')
@click.pass_context
        ...
        if use_sync:
            ...
            try:
                ...
                if not quiet and result.get('task_id'):
                    rich_print(f"✅ Learning task {result['task_id'][:8]}... queued for background processing", style="green")

                return

            except ImportError:
                if not quiet:
                    rich_print("⚠️  Async processing not available, using sync mode", style="yellow")
        ...
        with KuzuMemory(db_path=db_path) as memory:
            ...
            if not quiet:
                if memory_ids:
                    ...
                    rich_print("ℹ️  No memories extracted (content may be too generic)", style="yellow")
...
def recent(ctx, recent, output_format):
    ...
    try:
        ...
        with KuzuMemory(db_path=db_path) as memory:
            ...
            context = memory.attach_memories(
                prompt="recent project information",
                max_memories=recent
            )
            ...
            if output_format == 'json':
                ...
                if context.memories:
                    rich_print(f"📋 Recent {len(context.memories)} memories:", style="bold blue")
                    for i, memory in enumerate(context.memories, 1):
...
@cli.command()
@click.argument('content', required=True)
@click.option('--source', default='cli', help='Source of the memory (e.g., "conversation", "document")')
...
def remember(ctx, content, source, session_id, agent_id, metadata):
    ...
    try:
        ...
        with KuzuMemory(db_path=ctx.obj.get('db_path'), config=config) as memory:
            ...
            if memory_ids:
                rich_print(f"✅ Generated {len(memory_ids)} memories from your content!", style="green")

                if len(memory_ids) == 1:
                    rich_print("💡 Tip: Longer or more detailed text often produces more memories", style="dim")
                ...
                # Show next steps
                rich_print(f"\n🔍 Try: kuzu-memory recall 'what do you know about this project?'", style="cyan")
        ...
        rich_print("💡 Metadata should be valid JSON, e.g.: '{\"key\": \"value\"}'", style="yellow")
        sys.exit(1)
    except Exception as e:
        rich_print(f"❌ Error storing memory: {e}", style="red")
        if ctx.obj['debug']:
...
def setup(ctx, advanced):
    ...
    try:
        ...
        if RICH_AVAILABLE and advanced:
            max_recall_time = Prompt.ask(
                "Maximum recall time (ms)",
                default="100"
            )
            ...
            rich_print("Using default performance settings (use --advanced for custom)", style="dim")
        ...
        with KuzuMemory(db_path=db_path, config=config) as memory:
            ...
            if test_memory.strip():
                ...
                if memory_ids:
                    rich_print(f"✅ Successfully stored {len(memory_ids)} memories!", style="green")

                    # Test recall
                    context = memory.attach_memories("What do you know?", user_id="setup-user")
                    if context.memories:
                        rich_print(f"✅ Memory recall working! Found: {context.memories[0].content[:50]}...", style="green")
                else:
                    rich_print("⚠️  No memories extracted. Try more detailed text.", style="yellow")
            ...
            try:
                ...
                if test_memory.strip():
                    ...
                    rich_print(f"✅ AI enhancement working! ({len(enhancement['enhanced_prompt'])} chars)", style="green")
        ...
        if enable_cli:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    rich_print("💡 Install Kuzu CLI: https://docs.kuzudb.com/installation", style="yellow")
                    return
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with KuzuMemory(db_path=test_db, config=config) as memory:
                    ...
                    memory_ids = memory.generate_memories("Test optimization", user_id="test")
                    test_time = (time.time() - start_time) * 1000

                    adapter_type = "CLI" if config.storage.use_cli_adapter else "Python API"
                    rich_print(f"✅ {adapter_type} adapter working! Test completed in {test_time:.1f}ms", style="green")
...
@click.option('--format', 'output_format', default='enhanced',
              ...
              help='Output format')
@click.option('--explain-ranking', is_flag=True, help='Show detailed ranking explanation including temporal decay')
@click.pass_context
