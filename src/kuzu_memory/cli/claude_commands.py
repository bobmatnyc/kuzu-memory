...
from .cli_utils import rich_print, rich_panel, rich_confirm, console, RICH_AVAILABLE
    ...
    try:
        ...
        if not force:
            rich_panel(
                ...
                + (f"• Claude Desktop config (if detected)" if status['claude_desktop_detected'] else ""),
...
def run_mcp_server(ctx, project_root):
    ...
    try:
        import asyncio
        from ..integrations.mcp_server import main, MCP_AVAILABLE
