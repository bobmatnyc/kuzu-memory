...
class KuzuConnectionPool:
    ...
    def __init__(self, db_path: Path, pool_size: int = 5):
        if kuzu is None:
            raise DatabaseError("Kuzu is not installed. Please install with: pip install kuzu>=0.4.0")
...
class KuzuAdapter:
    ...
    def __init__(self, db_path: Path, config: KuzuMemoryConfig):
        ...
        self._pool = KuzuConnectionPool(
            db_path, 
            pool_size=config.storage.connection_pool_size
        )
    ...
    def _create_schema(self) -> None:
        ...
        try:
            from .schema import SCHEMA_DDL, INDICES_DDL, INITIAL_DATA_DDL
            ...
            with self._pool.get_connection() as conn:
                ...
                data_statements = [stmt.strip() for stmt in INITIAL_DATA_DDL.split(';') if stmt.strip()]
        ...
        try:
            with self._pool.get_connection() as conn:
                ...
                if self.config.performance.enable_performance_monitoring:
                    ...
                    if (self.config.performance.log_slow_operations and 
                        execution_time_ms > timeout_ms * 0.8):
    ...
    def cleanup_expired_memories(self) -> int:
        ...
        try:
            ...
            # Execute cleanup
            self.execute_query(
                get_query("cleanup_expired_memories"),
